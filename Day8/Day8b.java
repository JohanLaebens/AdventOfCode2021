...
//                    }
        ...
        globalcounter += Integer.parseInt(perline);
    }
    ...
    private void findWhichNumberIsWhich() {
        ...
        for (int i= 0;i<numbers1though10.length;i++) {
            if (numbers1though10[i].length() == 3) // nummer 7
            {
                ...
                String temp = number7.replace(number1.substring(0,1),"");
                a = temp.replace(number1.substring(1,2),"");
    ...
    if (!number6.contains("g") && (number8.contains("g")))
    {
        return "g";
    }
    for (int i=0;i<number8.length();i++)
